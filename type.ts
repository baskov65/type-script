...
const round: (v: number) => number = v => Math.round(v);
const pow: (p: number) => number = p => Math.pow(10, p);

type roundingType = (value: number, increment: number) => number;
const rounding: roundingType  = (value, increment) => round(value / increment) * increment;
 ...
 * @param {object} [opts]
 */
type currencyType = (value: number | string, opts: defaultsType) => void;

function currency(value: number | string, opts: defaultsType) {
    let that = this;
